...
class TaskCollapse extends Component{
    ...
    formSubmit(){  
      ...
      this.props.form.validateFields((err, values) => {
          var flag=false;
          if(values.reason){ //这里要自己手写验证，formItem嵌套formItem点同意的时候也会验证这个属性，会导致同意不通过
              this.setState({
                  message:""
              });
              flag=true;
                ...
                return(
                  <Panel header={col.name} key={self.props.data.length-index}>
                     <Form>
                        ...
                        >                
                            <Button type="danger" style={{marginLeft:5,width:100}} onClick={self.showModal.bind(self)}>退回</Button>
                                ...
                                >
                                    {getFieldDecorator('reason')( 
                                      <div>
                                        ...
                                        <span style={{color:'#F04134'}}>{self.state.message}</span>
                                      </div>
                                    )}
                                </FormItem>
          				          </Modal>
     	...
     	genProcessVarInfoRows(){
            ...
            if(row.variable.name=="自评分数"||row.variable.name=="考核小组打分"||row.variable.name=="部门评分"){
              ...
              <div style={conStyle}>
                    ...
                    <Col span="18" style={{textAlign:'left'}}>
                       <span style={scroreStyle}>{row.variable.value}</span>
                    </Col>
              </div>
            );
     		});
